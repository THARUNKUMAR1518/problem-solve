...
const Courses = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedCourse) {
        ...
        await api.post(`/courses?departmentId=${selectedDeptId}`, payload);
      }
      fetchCourses();
      handleCloseModal();
    } catch (err) {
      setError(err.response?.data?.message || 'Error occurred while saving course details.');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this course? This will soft-delete the record.')) return;
    try {
    ...
    <DashboardLayout navItems={isSuperAdmin ? superAdminNav : collegeAdminNav}>
      <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 class="text-2xl font-bold text-slate-900 font-sans">Courses / Programs</h1>
            <p class="text-sm text-slate-500 font-medium">Manage degree programs and curricular tracks inside departments.</p>
          </div>
          <button 
        ...
        {/* Filters Panel */}
        <div class="flex flex-col md:flex-row items-center gap-4 bg-white p-4 rounded-2xl border border-[#F1F5F9] shadow-premium">
          {isSuperAdmin && (
          ...
          <div class="flex-1 w-full relative">
            <label class="hidden md:block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 font-sans">Search Course</label>
            <div class="relative">
        ...
        {!selectedDeptId ? (
          <div class="bg-white rounded-2xl border border-slate-100 shadow-premium p-12 text-center">
            ...
            <p class="text-sm text-slate-500 font-medium">Please select a department to view courses.</p>
          </div>
        ) : loading ? (
          <div class="bg-white rounded-2xl border border-slate-100 shadow-premium p-12 text-center">
            <div class="w-10 h-10 border-3 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            <p class="text-sm text-slate-500 font-medium">Fetching courses...</p>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-slate-100 shadow-premium p-16 text-center">
            ...
            <p class="text-sm text-slate-500 max-w-sm mx-auto font-medium">Register a course under this department to map assessments.</p>
          </div>
        ) : (
              ...
              <table class="w-full text-left border-collapse">
                <thead>
                  <tr class="bg-slate-50 border-b border-slate-100">
                    <th class="px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Course Name</th>
                    <th class="px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Program Code</th>
                    <th class="px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Actions</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                  {currentItems.map((course) => (
                    <tr key={course.id} class="hover:bg-slate-50/50 transition-colors">
                      <td class="px-6 py-4 font-semibold text-slate-900">{course.name}</td>
                      <td class="px-6 py-4">
            ...
            {totalPages > 1 && (
              <div class="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <p class="text-xs text-slate-500 font-medium">
                  ...
                  <span class="font-bold">{Math.min(indexOfLastItem, filteredCourses.length)}</span> of{' '}
                  <span class="font-bold">{filteredCourses.length}</span> courses
                </p>
            ...
            <div class="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-premium overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-bold text-slate-900 text-base">
                  {selectedCourse ? 'Modify Course' : 'Create Course'}
                </h3>
              ...
              <form onSubmit={handleSubmit}>
                <div class="p-6 space-y-4">
                  ...
                  <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Course Name</label>
                    <input
                      type="text"
                      required
                      value={formName}
